...
func main() {
	...
	if err != nil {
		log.Fatalf("Invalid backup expiration '%s': %v\n", *expiration, err)
	}
	...
	if err != nil {
		log.Fatalf("Failed to retrieve servlets: %v\n", err)
	}
	...
	for _, s := range *servers {
		...
		if err != nil {
			log.Printf("Error creating backup of servlet '%s' named '%s': %v\n", s.Name, backupName, err)
			continue
		}
		log.Printf("Backup '%s' of servlet '%s' successfully created\n", s.Name, backupName)
	}
	backups, _, err := c.Backup.List()
	if err != nil {
		log.Fatalf("Failed to retrieve backups: %v\n", err)
	}
	for _, b := range *backups {
		if b.Status != "finished" {
			log.Printf("Skipping backup '%s': not finished\n", b.Name)
			continue
		}
		createdAt, err := time.ParseInLocation("02.01.2006 15:04:05", b.Created, time.Local)
		if err != nil {
			log.Printf("Skipping backup '%s': invalid creation time: %v\n", b.Name, err)
			continue
		}
		if createdAt.After(backupValidTill) {
			log.Printf("Skipping backup '%s': not expired\n", b.Name)
			continue
		}
		_, _, err = c.Backup.Remove(b.ID)
		if err != nil {
			log.Printf("Error removing backup '%s': %v\n", b.Name, err)
			continue
		}
		log.Printf("Backup '%s' successfully deleted\n", b.Name)
	}
}
